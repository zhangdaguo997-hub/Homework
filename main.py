...
# Common arguments
parser.add_argument('--model', type=str, default='deepseek-chat')  # 修改默认模型
...
# 请通过环境变量设置 DeepSeek API Key（不要把密钥写入代码）。
# 示例：在 PowerShell 中运行一次：
#   $env:DEEPSEEK_API_KEY = 'your_key_here'
# 或在当前终端会话中导出环境变量。在脚本运行前必须设置该变量。
if not os.getenv("DEEPSEEK_API_KEY"):
    print("Error: DEEPSEEK_API_KEY not set. Please set the environment variable before running.\nExample (PowerShell): $env:DEEPSEEK_API_KEY = 'YOUR_KEY'")
    exit(1)
